...
2. メインの `<script>`
   ...
   - 答え合わせのあと「次の問題へ」を押す前にページが読み込み直されたとき（Claudeアプリへ移ったときなど）は、答え合わせ済みの問題（`run.log` にある問題。まとめ解きは `WA.res`）をもう一度出さず次へ進める。最後まで済んでいれば「結果を見る」を出す。結果の点数は `run.log` から数え直す（同じ問題を二重に数えて満点が105点などにならないように）。二重に数えていた頃の周回の記録（満点が100点を超えるもの）は、`norm()` で読み込むときに100点満点（点数は100点まで）に直す
